...
class _ColourChangerStateFull extends State<ColourChanger>{
  ...
  Widget _buildColourChangerButton(colorNameStr, color){
    ButtonStyle btnStyle = ButtonStyle(
      ...
      textStyle: MaterialStateProperty.resolveWith((states) => const TextStyle(fontSize: 20))
    );

    String btnText = "Change to: " + colorNameStr + (color == thisStateColour ? "*" : "");
    return ElevatedButton(onPressed: () =>
        _setColourAction(color),
        child: Text(btnText),
        style: btnStyle);
  }
